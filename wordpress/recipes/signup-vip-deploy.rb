...
node[:deploy].each do |application, deploy|
    ...
    aws_key = (deploy[:signup_vip_app][:s3_access_key] rescue nil)
	aws_secret = (deploy[:signup_vip_app][:s3_secret_key] rescue nil)
	s3_region = (deploy[:signup_vip_app][:s3_region] rescue nil)
	bucket_name = (deploy[:signup_vip_app][:s3_bucket] rescue nil)
	local_dir = (deploy[:signup_vip_app][:local_dir] rescue nil)
